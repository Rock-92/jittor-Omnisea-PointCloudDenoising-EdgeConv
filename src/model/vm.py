...
def patch_based_denoise(model: VelocityModule, pcl_noisy, patch_size=1000, seed_k=6, seed_k_alpha=1) -> jt.Var:
    ...
    # KNN patches around FPS seeds are high coverage but not a formal guarantee.
    # Add one patch centered at each uncovered point so every input point is
    # predicted at least once and the output keeps exactly N points.
    covered = np.zeros(N, dtype=np.bool_)
    covered[point_idxs[0].detach().numpy().reshape(-1)] = True
    missing_idx_np = np.where(~covered)[0]
    if len(missing_idx_np) > 0:
        missing_idx = jt.array(missing_idx_np).int32()
        missing_seed_pnts = pcl_noisy[:, missing_idx, :]
        ...
        num_patches += len(missing_idx_np)
